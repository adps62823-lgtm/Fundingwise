...
@router.get("/workers")
async def list_workers(user=Depends(require_service("labor_management")), db=Depends(get_db)):
...
@router.post("/workers")
async def create_worker(payload: dict, user=Depends(require_service("labor_management")), db=Depends(get_db)):
...
async def update_worker(worker_id: str, payload: dict, user=Depends(require_service("labor_management")), db=Depends(get_db)):
...
async def list_assignments(project_id: str, user=Depends(require_service("labor_management")), db=Depends(get_db)):
...
async def create_assignment(payload: dict, user=Depends(require_service("labor_management")), db=Depends(get_db)):
    doc = {**payload, "org_id": user["organization_id"], "created_at": __import__("datetime").datetime.utcnow(), "updated_at": __import__("datetime").datetime.utcnow()}
...
async def update_assignment(assignment_id: str, payload: dict, user=Depends(require_service("labor_management")), db=Depends(get_db)):
