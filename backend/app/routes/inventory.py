...
@router.get("/items")
async def list_items(user=Depends(require_role("official")), db=Depends(get_db)):
...
@router.post("/items")
async def create_item(payload: dict, user=Depends(require_role("official")), db=Depends(get_db)):
...
async def update_item(item_id: str, payload: dict, user=Depends(require_role("official")), db=Depends(get_db)):
...
@router.get("/dispatch")
async def list_dispatch(project_id: str, user=Depends(require_role("official")), db=Depends(get_db)):
    items = await db.dispatches.find({"project_id": project_id, "org_id": user["organization_id"]}).to_list(length=1000)
    return {"data": [serialize_doc(item) for item in items], "error": None}


@router.post("/dispatch")
async def create_dispatch(payload: dict, user=Depends(require_role("official")), db=Depends(get_db)):
    doc = {**payload, "org_id": user["organization_id"], "created_at": __import__("datetime").datetime.utcnow(), "updated_at": __import__("datetime").datetime.utcnow()}
...
async def update_dispatch(dispatch_id: str, payload: dict, user=Depends(require_role("official")), db=Depends(get_db)):
